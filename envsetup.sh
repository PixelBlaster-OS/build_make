...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the PixelBlaster github
        T=$(gettop)
        cd $T > /dev/null
        vendor/blaster/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/blaster/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
